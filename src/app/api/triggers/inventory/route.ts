...
export async function POST(request: Request) {
  try {
    ...
    const evaluated = await createAndEvaluatePayoutIntent({
      ...
      reason: body.reason ?? "inventory_threshold_replenishment",
      source: "inventory_trigger",
      idempotencyKey: `inv_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
    });
    ...
    logAudit({
      actor: "system",
      action: "inventory_trigger_evaluated",
      result: executionError ? "failure" : "success",
      businessId: body.businessId,
      metadata: {
        trigger: body.trigger ?? "inventory_low",
    ...
    return errorResponse(400, unknownErrorToApiPayload(error, "inventory_trigger_failed"));
  }
}
